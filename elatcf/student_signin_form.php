			...
			<input type="text" class="input-block-level" id="username" name="username" placeholder="Student No" required>
			...
			jQuery("#signin_student").submit(function(e){
					...
					$.ajax({
						...
						if(html=='true')
						{
						$.jGrowl("Welcome to ELA Learning Management System", { header: 'Sign up Success' });
						var delay = 2000;
							setTimeout(function(){ window.location = 'dashboard_student.php'  }, delay);  
						}else if(html=='false'){
							$.jGrowl("student does not found in the database Please Sure to Check Your Student No or Firstname, Lastname and the Section You Belong. ", { header: 'Sign Up Failed' });
						}
						}
